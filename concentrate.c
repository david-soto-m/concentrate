...
int main(int argc, char const *argv[]) {
    ...
    FILE *pf;
    if (cont.exit == 1) {
        if (access(cont.exit_path, W_OK) == 0) {
            ...
            if (pf) {
                ...
                printf(
                    "ERROR adding exit instruction to %s\n",
                    cont.exit_path);
            }
        } else {
            printf(
                "ERROR %s not write accessible\n",
                cont.exit_path);
        }
    }
    ...
    sigprocmask(SIG_UNBLOCK, &sensible_set, NULL);
    // Ignores every other signal
    sigact action_ctrl_c = {
        .sa_handler = user_inter_handler,
        .sa_flags = 0
    };
    sigact action_time = {.sa_flags = SA_SIGINFO};
    sigaction(SIGINT, &action_ctrl_c, NULL);
    // Handles control C interactions
    sigaction(SIGRTMIN+2, &action_time, NULL);

    user_inter_handler(&cont);

    struct itimerspec spec = {
        .it_value = cont.timerep,
        .it_interval = cont.timerep,
    };
    struct sigevent event = {
        .sigev_signo = SIGRTMIN+2,
        .sigev_notify = SIGEV_SIGNAL,
        .sigev_value.sival_int = 1,
    };
    timer_t timer_rep;
    char noerror = 0;
