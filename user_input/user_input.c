...
void user_inter_handler(){
	...
	//REGION TIMER BUILD
		timer_t temp;
		struct timespec time1={.tv_sec=10,.tv_nsec=0},time0={.tv_sec=0,.tv_nsec=0};
		struct timespec time_curr_act;
		clock_gettime(CLOCK_REALTIME,&time_curr_act);
		struct itimerspec a={.it_value=time1,.it_interval=time0,};
	...
	//ENDREGION
	timer_settime(temp,0,&a,NULL);//runs timer to cancel scaning for a long time
	...
	if (timesig==1){
		...
		if(strcmp(buff,"t")==0){
			...
			while(time_rest){
				time_rest=sleep(time_rest);//It will recieve the interruptions of the regular killer timer
			}
		}
